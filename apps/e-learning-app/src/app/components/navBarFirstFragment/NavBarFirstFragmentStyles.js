...
const navBarFirstFragmentStyles = {
  ...
  toggleMaterialsButton: (windowHeight) => ({
    ...
    height:
      (navBarHeight * windowHeight) / 100 >= minNavBarHeight
        ? `${materialsButtonHeight}vh`
        : `${minMaterialsButtonHeight}px`,
    top:
      (navBarHeight * windowHeight) / 100 >= minNavBarHeight
        ? `${materialsButtonTop}vh`
        : `${minMaterialsButtonTop}px`,
    zIndex: '90',
    border: '0px',
  }),
  ...
  dropDownIcon: (windowHeight) => ({
    ...
    height:
      (navBarHeight * windowHeight) / 100 >= minNavBarHeight
        ? `${dropdownIconHeight}vh`
        : `${minDropdownIconHeight}px`,
    top:
      (navBarHeight * windowHeight) / 100 >= minNavBarHeight
        ? `${dropdownIconTop}vh`
        : `${minDropdownIconTop}px`,
    zIndex: '89',
  }),
